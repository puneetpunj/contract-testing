...
describe("Pact", () => {
    ...
    // const animalListExpectation = animalBodyExpectation
    ...
    describe("when a call to list all animals from the Animal Service is made", () => {
        describe("and the user is not authenticated", () => {
            ...
            it("returns a 401 unauthorized", async () => {
                // console.log()
                // suggestion(suitor).then(o=>{
                //     console.log('hello')
                // }).catch(err=> console.log('i am here'+ err))
                // return expect(suggestion(suitor)).to.eventually.be.rejectedWith("Unauthorized")
                return expect((availableAnimals())).to.eventually.be.rejectedWith("Unauthorized")
                // availableAnimals().then(a => console.log).catch(err => console.log('hello ' + err))
        ...
        describe("and the user is authenticated", () => {
            describe("and there are animals in the database", () => {
                ...
                it("returns a list of animals", done => {
                    ...
                    const suggestedMates = availableAnimals();
                    availableAnimals().then(a => {
                        // console.log(a)
                        expect(a.length).to.not.equal(9);
                        done();
                    })
                    // expect(suggestedMates).to.eventually.have.deep.property("id", 11)
                    // expect(suggestedMates).to.eventually.have.nested.property(
                    //     "suggestions[0].score",
                    //     95
                    // )
